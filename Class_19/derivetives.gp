...
plot [0:2*pi] sin(x), \
              cos(x) title "cos(x)", \
              "derivetives.dat" every ::1 using 1:2 with lines title "sin(x) (from file)", \
               "derivetives.dat" every ::1 using 1:3 with lines title "cos(x) (from file)", \
              "derivetives.dat" every ::1 using 1:4 with lines title "sin'(x) (finite difference)"
